...
>
    ...
    >
        End Date
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger className="w-fit" asChild><Button variant="dropDown" className="bg-orange-500 text-white">Choose legend</Button></DropdownMenuTrigger>
                    <DropdownMenuContent className="flex flex-col gap-y-1">
                        {filterLabels.length !== 0
                            ? filterLabels.map(label => (
                                <DropdownMenuItem key={label} onClick={() => setLegend(label)} className={cn(legend === label && "bg-blue-600 text-white focus:bg-blue-600 focus:text-white focus:bg-opacity-90")}>{SpacedNamed(label)}</DropdownMenuItem>
                            ))
