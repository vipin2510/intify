...
const SCHEMIFY_SPREADSHEET_ID = process.env.SCHEMIFY_SPREADSHEET_ID;
...
export const getSpreadsheetData = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(500)
      .send(err.message || "Error occured while fetching spreadsheet");
  }
};
...
export const getSheetData = async (req: Request, res: Response) => {
  ...
  try {
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: SCHEMIFY_SPREADSHEET_ID,
      range,
    });
...
export const getSheetNames = async (req: Request, res: Response) => {
  ...
  try {
    res.json({
      data: sheetnames.data.sheets?.map((s) => s.properties?.title) || [],
    });
  } catch (err: any) {
