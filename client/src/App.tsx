    ...
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const response = await fetch(
          "https://intify-server.vercel.app/api/verify-token",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
                ...
                <>
                  ...
                  <div className="absolute top-0 left-0 m-4 z-20 controls-box">
                    <div className="flex gap-x-2">
                      ...
                      />
                      <label htmlFor="enable-markers" className="text-sm">
                        Markers
                      </label>
                    </div>
                    <div className="flex gap-x-2">
                      <input
                        ...
                        type="checkbox"
                        id="enable-border"
                        checked={showLayer.border}
                      />
                      <label htmlFor="enable-border" className="text-sm">
                        Borders
                      </label>
                    </div>
          ...
          {!isAuthenticated && (
            <div className="z-[999] absolute w-screen h-screen backdrop-blur-md bg-black/60 flex flex-col justify-center items-center">
              <section className="bg-white/80 dark:bg-black/70 p-4 gap-y-3 lg:w-1/3 sm:w-1/2 w-full rounded-md flex flex-col justify-center items-center">
                <h1 className="md:text-4xl text-2xl text-blue-500 font-[Viga] uppercase">
                  Intify
                </h1>
                <h3 className="md:text-lg text-red-500 text-center">
                  Only authorised people are allowed. Verify by logging in with
                  your Google account.
                </h3>
                <button
                  onClick={() => login()}
